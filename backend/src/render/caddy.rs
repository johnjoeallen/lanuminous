...
pub struct CaddyRenderer;

impl Renderer for CaddyRenderer {
    ...
    fn render(&self, site: &SiteConfig) -> Result<Vec<RenderedArtifact>> {
        if site.reverse_proxies.sites.is_empty()
            || !matches!(site.reverse_proxies.provider, ReverseProxyProvider::Caddy)
        {
            return Ok(Vec::new());
        }
        ...
        for proxy in &site.reverse_proxies.sites {
            ...
            lines.push(format!("{} {{", proxy.server_names.join(", ")));
            lines.push(format!(
                "    reverse_proxy {}:{}",
